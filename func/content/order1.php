...
if (isset($_POST['order'])) {
	...
	if (empty($post_service) || empty($post_link) || empty($post_quantity)) {
		...
		// api data
			...
			if ($provider == "MANUAL") {
			...
			} else if ($provider == "BOSPANEL") {
                $postdata = "api_token=$api_key&action=add&package=$pid&link=$post_link&quantity=$post_quantity";
                $ch = curl_init();
                curl_setopt($ch, CURLOPT_URL, 'https://bospanel.com/api/v2');
                curl_setopt($ch, CURLOPT_POST, 1);
                curl_setopt($ch, CURLOPT_POSTFIELDS, $postdata);
                curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
                curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, false);
				...
				if ($insert_order == TRUE) {
					$msg_type = "success";
					$msg_content = "<b>Success</b><br /><b>Pesanan Berhasil</b><br /><b>ID Pesanan:</b> $oid<br /><b>Layanan:</b> $service<br /><b>Link:</b> $post_link<br /><b>Total:</b> ".number_format($post_quantity,0,',','.')."<br /><b>Harga:</b> Rp. ".number_format($price,0,',','.');
				} else {
					$msg_type = 'error';
...
Jika terjadi kesalahan / mendapatkan pesan gagal yang kurang jelas, silahkan hubungi Admin untuk informasi lebih lanjut.</li><br />
 ...
 <li>Jika Anda memasukkan orderan di kiosseleb berarti Anda sudah mengerti aturan kiosseleb dan <b><a href="?<?php echo paramEncrypt('content=ketentuan')?>">jangan lupa baca menu F.A.Q serta Terms</a></li></b>
